...
public class AppException implements Thread.UncaughtExceptionHandler {
    ...
    private static class InstanceHolder {
        static AppException handler = new AppException();
    }
    ...
    private void saveFile(Throwable throwable) {
        ...
        if (Environment.MEDIA_MOUNTED.equals(state)) {
            String storagePath = Environment.getExternalStorageDirectory().getAbsolutePath() + File.separator + mContext.getPackageName();
            boolean canWrite = Environment.getExternalStorageDirectory().canWrite();
            AppLogUtil.log("canWrite = " + canWrite);
            ...
            try {
                ...
                AppAutoClose.quietClose(fileWriter1);
            }
        }
    }
